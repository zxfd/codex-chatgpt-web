...
## このフォーク：GPT-6 Pro とローカルモデル一覧

既存のモデル ID `chatgpt-web/pro` は、Sol の Pro effort の表示名を変えるだけでなく、
**GPT-6 Pro（Astra）** に明示的にルーティングされます。ブラウザーで GPT-6 モデルを選択・確認してから
Pro effort を設定し、プロンプト送信前にモデルを再確認します。アカウントへの提供がまだ始まっていない場合、
選択肢が無効な場合、または UI で確認できない場合は明示的なエラーとなり、GPT-5.6 Sol Pro に黙って切り替えません。
正式名称と提供状況は公式の説明を参照してください：
[GPT-5.6 and GPT-6 Pro in ChatGPT](https://help.openai.com/en/articles/20001354-gpt-56-in-chatgpt)。

ローカル Codex の一覧ではネイティブの **Luna と Terra** を非表示にし、Sol とネイティブ Astra は維持します。
ネイティブプロトコルのメタデータは削除せず、実行中のタスクや保存済みの既定モデルも変更しません。
Free/Go 用の Web Luna 互換ルートは引き続き利用できます。有料アカウントには主に Web High、
Web Extra High（Pro が必要）、Web GPT-6 Pro（Pro と Web での利用権限が必要）が表示されます。

このフォークのソースを更新してランチャーを再起動し、Codex を完全に再起動してモデル一覧を更新した後、
**ChatGPT Web — GPT-6 Pro** を選択してください。`git pull` だけではインストール済みの上流版は更新されません。
ソースから実行する場合は `bun run app` を使用し、パッケージ版はこのフォークから再ビルドしてください。
GPT-6 には当面、API のコンテキスト上限ではなく、既存の Pro の保守的なブラウザー上限を使用します。
実際の Web 上限は実測による確認が必要です。
